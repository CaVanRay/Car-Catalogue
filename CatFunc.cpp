...
void editCar(car* n) {
    ...
    do {
        ...
        case 3: {
            std::string newVin = getVIN("Enter VIN: ", head);
            n->SetVin(newVin);
            break;
        }
        case 4: {
...
void moveCar(car*& head, car*& tail, std::vector<car*>& carCatalogue, car*n){
	...
	*/
	...
	int newindex = getInt("Enter desired new index for car: ");
		...
		while (newindex < 1 || newindex > listSize || newindex == n->GetIndex) {
			if(newindex < 1 || newindex > listSize){
				newindex = getInt("Invalid index, Try again: ");
			}else{
				newindex = getInt("requested index is current index, Try again: ");
			}
		}
	...
	if(newindex == 1){
		n->SetNext(head);
		// Make sure head currently exists
		// If it does, set head's prev pointer to n
